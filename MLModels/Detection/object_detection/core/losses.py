    ...
    def _compute_loss(self,
        ...
        if class_indices is not None:
            weights *= tf.reshape(
                ...
                [1, 1, -1])
        # classes_weights = tf.constant([1.0, 0.05, 0.27, 1.0])
        # classes_weights = tf.constant([1.0, 1.0, 5.85, 22])
        classes_weights = tf.constant([1.0, 1.0])
        per_entry_cross_ent = (tf.nn.weighted_cross_entropy_with_logits(
            targets=target_tensor, logits=prediction_tensor, pos_weight=classes_weights))
